...
public:
    ...
    void run(){
        ...
        Enemy *enemy4 = new Enemy(sheetEnemy);
        enemy2->spriteRect->x = 520;
        enemy2->spriteRect->y = 96;
        enemy3->spriteRect->x = 520;
        enemy3->spriteRect->y = 96;
        enemy4->spriteRect->x = 416;
        enemy4->spriteRect->y = 64;
        ...
        drawables.push_back(enemy4);
        movables.push_back(enemy4);
        kolizije.push_back(enemy);
        kolizije.push_back(enemy2);
        kolizije.push_back(enemy3);
        kolizije.push_back(enemy4);
        ...
        while(running){
            ...
            for(size_t i=0; i<kolizije.size();i++){
                   bool p = sp->provera(kolizije[i]);
                    if(p == true){
                        if(kolizije[i]->isKey()){
                            drawables.erase(drawables.begin()+7);
                            kolizije.erase(kolizije.begin()+5);
                            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION,"Cestitamo:","Pokupili ste kljuc!",window);
                            cout<<"Pokupili ste kluc!"<<endl;
                            imaKljuc=true;
                            cout<<imaKljuc<<endl;
                        }
                        else if(kolizije[i]->isChest() && imaKljuc==true){
                            SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION,"Cestitamo:","Pobedili Ste !",window);
                            running = false;
                        }
                        else if(kolizije[i]->isEnemy()) {
                        SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION,"UPS!","Ubio vas je cuvar!",window);
                            running = false;
                            }
                    }
            }
